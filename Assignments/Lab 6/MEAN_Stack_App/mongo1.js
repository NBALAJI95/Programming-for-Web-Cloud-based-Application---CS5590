...
app.get('/get', function (req, res) {
    MongoClient.connect(url, function(err, db) {
        ...
        db.collection('books1').find().toArray(function(err, result){
            if(err)
            {
...
app.get('/delete/:toBeDeleted_id', function (req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        if(err)        {
            res.write("Failure");
            res.end();        }
        db.collection('books1').deleteOne({"_id": ObjectID(id)}, function(err, result) {
...
app.get('/update/:toBeUpdated_id', function (req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        var newvalues={"grades":req.query.grades, "lastName":req.query.lastName, "firstName":req.query.firstName, "emailID":req.query.emailID, "address":req.query.address};
        db.collection('books1').updateOne(myquery, newvalues, function(err, result) {
            if(err)            {
                res.write("Failed");
                res.end();            }
...
var insertDocument = function(db, data, callback) {
    db.collection('books1').insertOne( data, function(err, result) {
        if(err)
        {
